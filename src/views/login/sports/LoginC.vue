      ...
      <section class="brand-panel">
        ...
        <div class="brand-visual">
          <div class="illustration-card">
            <img class="brand-illustration" src="@/assets/img/login/sports/login-pic.png" alt="智慧场馆插画" />
          </div>
        </div>
      </section>
...
.decor-glow { position: absolute; width: 40vw; height: 40vw; border-radius: 50%; filter: blur(60px); opacity: 0.35; pointer-events: none; }
.decor-glow--tl { top: -10vw; left: -10vw; background: radial-gradient(closest-side, #60a5fa, transparent); }
.decor-glow--br { right: -10vw; bottom: -10vw; background: radial-gradient(closest-side, #0ea5e9, transparent); }

.login-layout { width: min(88vw, calc(92vh * (16 / 9))); aspect-ratio: 16 / 9; height: auto; display: grid; grid-template-columns: 1.25fr 1fr; gap: 32px; padding: 32px; position: relative; z-index: 1; }
...
.brand-visual { flex: 1; display: flex; align-items: center; justify-content: center; }
.illustration-card { width: 100%; max-width: 820px; padding: 18px; border-radius: 16px; background: rgba(255, 255, 255, 0.65); border: 1px solid rgba(255, 255, 255, 0.8); box-shadow: 0 18px 40px rgba(13, 70, 160, 0.22); backdrop-filter: blur(6px); }
.brand-svg { width: 100%; max-width: 760px; height: auto; }
...
.form-item label { font-size: 14px; color: #1f2937; }
.form-item input { height: 40px; padding: 0 12px; border-radius: 10px; border: 1px solid rgba(59, 130, 246, 0.18); background: rgba(255, 255, 255, 0.92); color: #0f172a; outline: none; }
.form-item input:focus { border-color: #3b82f6; box-shadow: 0 0 0 4px rgba(59, 130, 246, 0.18); }
...
@media (max-width: 1024px) {
  .login-layout { width: min(96vw, calc(96vh * (16 / 9))); padding: 20px; grid-template-columns: 1fr; gap: 20px; }
  .brand-panel { order: 2; }
  .form-panel { order: 1; }
}

/* 方案C：蓝紫智感（品牌感）覆盖样式（保留） */
.login-page { background: radial-gradient(60% 60% at 50% 50%, #0F1436 0%, #13295E 70%, #0b3ba7 100%); }
.decor-grid { opacity: 0.04; }
.login-card::before { background: linear-gradient(135deg, rgba(79, 70, 229, 0.6), rgba(37, 99, 235, 0.45)); }
.submit-btn { background: linear-gradient(135deg, #4F46E5, #2563EB); }
.submit-btn:hover { background: linear-gradient(135deg, #4338CA, #2563EB); }
</style>
